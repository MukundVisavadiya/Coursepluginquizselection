...
jQuery(document).on("click", ".as-navbar-toggler", function () {
  ...
  if (jQuery('nav').hasClass('active')) {
    jQuery('.as-navbar-toggler i').removeClass('fa-angle-right').addClass('fa-angle-left');
    jQuery('.as-sildebar-chapter-list').show();
  } else {
    jQuery('.as-navbar-toggler i').removeClass('fa-angle-left').addClass('fa-angle-right');
...
jQuery(document).on("click", ".as-sidebar-chapter-accordion-parent", function () {
  var dataChapterId = jQuery(this).data("chapter-id");
  jQuery(this).toggleClass('active');
  jQuery(`.as-sildebar-chapter-list .as-lesson-accordion-sidebar-child-${dataChapterId}`).slideToggle();
...
jQuery(document).on("click", ".as-lesson-accordion-sidebar-child", function () {
  var dataLessonId = jQuery(this).data("lesson-id");
  jQuery(this).toggleClass('active');
  jQuery(`.as-topic-accordion-sidebar-grandchild-${dataLessonId}`).slideToggle();
...
jQuery(document).on("click", ".as-topic-accordion-sidebar-grandchild", function () {
  var dataTopicId = jQuery(this).data("topic-id");
  jQuery(this).toggleClass('active');
  jQuery(`.as-section-accordion-sidebar-grand-grandchild-${dataTopicId}`).slideToggle();
...
jQuery(document).ready(function () {
  ...
  function checkInputs(step) {
    const inputs = jQuery(step).find('input[type="radio"], input[type="checkbox"]');
    ...
    if (finishButton.length) {
      finishButton.on('click', function () {
        ...
        steps.each(function (index, step) {
          ...
          const selectedAnswers = jQuery(step).find('input:checked').map(function () {
            ...
            if (response.success === true) {
              ...
              < div class="as-question-feedback" >
                  ...
                  return `<li class="correct-answer" style="color: green; border: 1px solid;padding: 10px;margin-bottom: 10px;"><b>Correct Answer:</b> ${correctAnswer}</li>`;
                              ...
                              ${feedback.selected_answers.map(function (selectedAnswer) {
                  const isCorrect = feedback.correct_answers.includes(selectedAnswer);
                  return `<li class="${isCorrect ? 'correct-answer' : 'wrong-answer'}" style="color: ${isCorrect ? 'green' : 'red'}; border: 1px solid;padding: 10px;margin-bottom: 10px;"><b>Your Answer:</b> ${selectedAnswer}</li>`;
                }).join('')}
                          </ul>
  ...
  jQuery('input[type="radio"]').change(function () {
    jQuery('input[name="' + jQuery(this).attr('name') + '"]').parent().parent().removeClass('active');
